...
async function bootstrap(): Promise<void> {
  ...
  await queues.get('incremental-sync').upsertJobScheduler('source-deletion-reconciliation', { pattern: config.get('RECONCILIATION_CRON', { infer: true }) }, { name: 'incremental-sync', data: { scope: 'reconcile' }, opts: { removeOnComplete: 1000, removeOnFail: false } });
